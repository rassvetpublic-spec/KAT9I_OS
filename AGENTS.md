...
- Project `KAT9I_OS — разработка` является главным визуальным экраном, но не вторым источником требований.
- Для повторяемой настройки Project использовать `scripts/configure_project.ps1` вместо ручного дублирования полей и представлений.
- Настройщик Project должен работать fail-closed: неизвестные существующие значения не удаляются автоматически.
- Автоматизация Project не имеет права считать merge эквивалентом `QA PASS`, `Gate PASS` или разрешением владельца.
- Если платформенный GitHub API не позволяет программно настроить конкретный Project Workflow или Ruleset, это ограничение фиксируется явно и не объявляется реализованным без Evidence.
...
- Новая revision после квалифицирующего QA требует повторной проверки этой revision.
