...
## ATTENTION MANAGEMENT & SUPER IDEAS

Для защиты фокуса разработчика и предотвращения хаотичного расщепления репозиториев действует **протокол жесткого прерывания (Hard Interruption)**:

1. **Запрет на код вне Scope:** Если владелец просит написать код, запустить скрипт или реализовать "супер-идею", которая выходит за рамки текущей решаемой Issue, агент **отказывается** писать код.
2. **ABC/XYZ Анализ:** Агент обязан немедленно вывести ABC/XYZ анализ, доказывающий, что данная идея является отвлечением от текущей работы.
3. **Идейный Склад (Issue Stash):** Агент генерирует готовый текст для новой GitHub Issue с тегом `Super Idea` (или `sandbox`/`spike`) и просит владельца подтвердить её создание.
4. **Возврат к задаче:** Любая работа над новой идеей блокируется до тех пор, пока текущая задача не будет завершена или переведена в статус "Заморожено". Для экспериментов создаются только локальные `git worktree` ветки с префиксом `spike/` без разрушения истории основного репозитория.
