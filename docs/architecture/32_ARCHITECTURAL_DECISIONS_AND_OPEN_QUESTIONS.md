...
## 32.51. Открытые вопросы, которые нужно закрыть до Architecture Baseline

Следующие вопросы необходимо решить до начала полномасштабной реализации.

## 32.52. OQ-001 — основной язык/стек Core

Нужно определить:

> на каком основном языке реализуется Core и большинство системных модулей.
...
Нужно выбрать один канонический способ описания:
...
Должен быть закрыт до активной реализации контрактов.
...
Нужно определить физический SSoT для машинных контрактов.

Например логически:

`schemas/`

Но точная структура будет принята при реализации.
...
Нужно определить машинный способ хранить слои так, чтобы:

> не создать пять независимых копий текста.

Статус:

`OPEN BEFORE HTML/Electron DOCS GENERATOR`.

## 32.67. OQ-015 — интерфейс архитектурного словаря

Нужно определить, как одна запись термина хранит:
...
Архитектура ведения проекта принята.

Остаётся фактически настроить:
...
Это уже не архитектурный вопрос, а задача реализации GitHub-инфраструктуры.
