...
## 32.40k. ADR-044 — высокопроизводительный CacheEngine на Rust, RAM-first и политика Spill

**Статус:** ACCEPTED (канонический источник: [docs/architecture/34_CACHE_ENGINE.md](34_CACHE_ENGINE.md), Issue #84).

1. **Отдельный процесс/модуль на Rust**:
   - `CacheEngine` реализуется на Rust как отдельный сервис системного ядра;
   - Electron категорически исключён из hot path (`GET`/`PUT` запросов кэша), доступ из UI возможен только асинхронно через Core для чтения метрик и административных команд.
2. **RAM-first архитектура и batch spill на SSD**:
   - Основной рабочий режим — хранение записей в оперативной памяти с субмиллисекундной задержкой;
   - SSD не является штатным постоянным уровнем: сброс на диск выполняется крупными batch-операциями в неизменяемые (`immutable`) spill-сегменты с контрольными суммами только при наступлении `memory pressure`;
   - Минимизируются паразитный износ SSD, системные вызовы и write amplification.
3. **Кэш не является источником истины (Zero SSoT Violation)**:
   - В CacheEngine строго запрещено хранить единственную каноническую копию критических данных (`TaskRuntimeState`, `RulesRef`, `KnowledgeRecord`, `Evidence`, `SecretRef`, Git commits);
   - Полное удаление всего хранилища кэша может снизить производительность системы, но не должно нарушить её корректность.
4. **Контракты и вектор развития (Full architecture, minimal implementation)**:
   - В первой версии реализуются: RAM cache, структурированный `CacheKey`, детерминированный eviction (TTL/LRU), `SingleFlight` коалесценция, immutable spill segments, lazy restore, базовые метрики и degraded/fail-open поведение при сбоях;
   - Архитектурные контракты изначально проектируются с запасом совместимости (versioned protocol, capability negotiation, `PayloadRef`) под будущие расширения (Shared Memory/Zero-Copy, deduplication, mmap, compression, namespaces quotas) без ломки API Core/Context/Worker.
