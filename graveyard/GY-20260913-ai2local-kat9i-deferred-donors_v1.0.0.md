...
### 1.1. Термины этого снимка

Ниже зафиксированы краткие русские пояснения к англоязычным техническим терминам, которые используются дальше:

- `baseline` — исходный уровень измерений для последующего сравнения;
- `stage-router` — маршрутизатор, выбирающий модель или путь выполнения по стадии работы; `escalation` — эскалация к более сильной модели; `advisor-gate` — контрольный шлюз для обращения к модели-советнику;
- `runtime` — среда выполнения; `telemetry` — телеметрия; `adaptive routing` — адаптивная маршрутизация; `cost/quality/rework` — стоимость, качество и объём переделок;
- `spinning/looping` — зацикливание без полезного прогресса; `retry/rework` — повторные попытки и переделки; `capability gap` — недостаток требуемых возможностей;
- `crawling` — автоматический обход источников; `reranking` — повторное ранжирование результатов; `bounded retrieval` — ограниченное извлечение релевантного контекста; `context budget` и `token budget` — бюджет контекста и токенов;
- `retrieval` — извлечение контекста; `source refs` — ссылки на источники; `targeted expansion` — точечное расширение найденного контекста; `smoke test` — быстрая базовая проверка работоспособности;
- `memory engine` — движок памяти; `memory hit` — найденное совпадение в памяти; `current-canon verification` — проверка по текущему канону; `read-only experiment` — эксперимент без права записи;
- `token-efficient harness` — экономный по токенам тестовый каркас; `system prompt` — системная инструкция; `token-awareness` — явный учёт расхода токенов; `agent framework` — каркас для агентных систем; `input context` — входной контекст;
- `benchmark` — сравнительное измерение; `gap` — подтверждённый пробел в возможностях; `coding-agent runtime` — среда выполнения агента для работы с кодом; `durable sessions` — устойчивые восстанавливаемые сессии; `repository map` — карта структуры репозитория;
- `verification` — проверка результата; `skills/plugins/hooks` — навыки, плагины и хуки; `worktrees` — отдельные рабочие деревья Git; `usage telemetry` — телеметрия использования; `backend` — исполнительная серверная часть; `socket` — интерфейс подключения;
- `governance` — управление полномочиями и правилами; `hard floors` — жёсткие минимальные ограничения; `approval provenance` — происхождение одобрения; `earned autonomy` — автономность, расширяемая только после подтверждённой надёжности; `Capability` — техническая возможность; `Permission` — разрешение;
- `fail-closed` — безопасный отказ при неопределённости; `audit trail` — аудиторский след; `authority` — управляющее полномочие; `sandbox` — изолированная среда;
- `GUI Worker` — агент, работающий через графический интерфейс; `side effects` — внешние побочные изменения; `prompt injection` — внедрение вредоносных инструкций через данные; `evaluator` — проверяющий механизм;
- `gateway` — шлюз доступа; `alias` — псевдоним; `provider` — поставщик модели или сервиса; `scheduler` — планировщик; `account pool` — пул учётных записей; `proxy` — промежуточный сетевой узел;
- `OpenAI-compatible` — совместимый с интерфейсом OpenAI API; `provider-neutral` — не привязанный к конкретному поставщику; `persistent cache` — сохраняемый между запусками кэш; `speculative decoding` — опережающее декодирование; `bootstrap` — начальная развёртка; `dependency` — внешняя зависимость.
