...
Точный реестр, количество архивов и контрольные суммы находятся только в `MANIFEST.json` и не дублируются здесь вручную. Содержимое архивов может намеренно включать устаревшие правила или статусы и не является CONTROL.