...
def status_name(project: dict[str, Any]) -> str:
    matches = [
        ...
        if field.get("name") in {"Статус", "Status"}
    ]
    if len(matches) != 1:
...
def canonical_views(policy: dict[str, Any], actual_status_name: str) -> list[dict[str, str]]:
    return [
        {
            "name": str(view["name"]),
            "layout": str(view["layout"]),
            "filter": str(view["filter"]).replace("{status}", actual_status_name),
        }
...
def preflight(
    ...
    if unexpected:
        raise RuntimeError(
            "Project содержит неизвестные или регистрово отличающиеся представления: " + ", ".join(unexpected)
        )
...
def assert_canonical_complete(actual_views: list[dict[str, Any]], expected_views: list[dict[str, str]]) -> None:
    ...
    for expected in expected_views:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"После создания представление '{expected['name']}' существует неоднозначно: экземпляров {len(matches)}"
            )
        actual = matches[0]
        if actual.get("layout") != expected["layout"] or str(actual.get("filter") or "") != expected["filter"]:
...
def reconcile(owner: str, project_number: int) -> None:
    ...
    if len(final_views) != len(expected):
        extras = sorted(str(view.get("name")) for view in final_views if str(view.get("name")) not in {e["name"] for e in expected})
        raise RuntimeError(
