...
class StandardProjectBootstrapTests(unittest.TestCase):
    ...
    def test_bootstrap_has_read_only_status_and_no_merge_authority(self):
        ...
        self.assertIn("Preflight-RequiredSecrets $Manifest", text)
        self.assertIn("Ensure-AutomationCapabilities $Manifest $resolvedProject", text)
        self.assertIn("Set-SecretFromEnvironment", text)
        self.assertNotIn("optional_secret_names", text)
        self.assertNotIn("--body $material", text)
        lowered = text.lower()
