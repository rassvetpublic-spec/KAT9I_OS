...
class ProjectCredentialPreflightTests(unittest.TestCase):
    def test_contract_is_separate_read_only_preflight(self):
        ...
        self.assertIn("if($preflightLibraryMode){return}", preflight)
        self.assertIn("Resolve-PreflightProjectId", preflight)
        self.assertIn("Resolve-PreflightProjectItemId", preflight)
        self.assertIn("project_id=$projectId", preflight)
        self.assertIn("item_id=$itemId", preflight)
        self.assertNotIn("project item-edit", preflight)
        self.assertNotIn("'project','view'", preflight)
        self.assertNotIn("'project','item-list'", preflight)
    ...
    def test_workflow_runs_preflight_before_sync(self):
        ...
        self.assertIn("id: project_preflight", text)
        self.assertIn("steps.project_preflight.outputs.project_id", text)
        self.assertIn("steps.project_preflight.outputs.item_id", text)
