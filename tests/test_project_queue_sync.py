...
class ProjectQueueSyncTests(unittest.TestCase):
    def test_queue_policy_is_canonical(self):
        ...
        self.assertIn("[string]$ProjectId=''", sync)
        self.assertIn("[string]$ItemId=''", sync)
        self.assertIn("$hasProjectId -xor $hasItemId", sync)
        self.assertIn("$resolvedProjectId=$ProjectId.Trim()", sync)
        self.assertIn("$resolvedItemId=$ItemId.Trim()", sync)
    ...
    def test_workflow_is_trusted_and_never_merges(self):
        ...
        self.assertIn('-ProjectId "$env:PROJECT_ID"', text)
        self.assertIn('-ItemId "$env:PROJECT_ITEM_ID"', text)
