...
(function () {
  ...
  function createMenuDOM() {
    ...
    menu.innerHTML = `
      ...
      <div class="agy-menu-item" id="agy-action-toggle-matrix">
        <div class="agy-menu-item-left">
          <span class="agy-menu-icon">🟢</span>
          <span class="agy-menu-label">Эффект матрицы</span>
        </div>
        <span class="agy-menu-shortcut" id="agy-matrix-status-label">Вкл</span>
      </div>

      <div class="agy-menu-item" id="agy-action-settings">
        <div class="agy-menu-item-left">
          <span class="agy-menu-icon">⚙️</span>
          <span class="agy-menu-label">Настройки и кастомизация</span>
        </div>
      </div>
  ...
  // Загрузка настроек матрицы в модальное окно
  function loadModalMatrixSettings() {
    try {
      const toggle = document.getElementById('agy-matrix-toggle');
      const mode = document.getElementById('agy-matrix-mode');
      const intensity = document.getElementById('agy-matrix-intensity');

      let cfg = { enabled: true, mode: 'dynamic', intensity: 'medium' };
      if (window.AgyMatrixEffect && typeof window.AgyMatrixEffect.getConfig === 'function') {
        cfg = window.AgyMatrixEffect.getConfig();
      } else {
      ...
      if (mode) mode.value = cfg.mode || 'dynamic';
      if (intensity) intensity.value = cfg.intensity || 'medium';
    } catch (e) {
      console.warn('[AGY-UI] Ошибка загрузки настроек матрицы в модалке:', e);
    }
  }
  ...
  function createSettingsModalDOM() {
      ...
      <div class="agy-modal-card">
        <div class="agy-modal-header">
          <div class="agy-modal-title">
            <span>⚙️</span> Настройки интерфейса и кастомизации
          </div>
          <button class="agy-modal-close" id="agy-modal-close-btn">&times;</button>
        </div>
        <div class="agy-modal-body">
          <div class="agy-modal-section-title">
            <span>🟢</span> Визуальный эффект матрицы (Digital Rain & Liquid Flow)
          </div>
          <div class="agy-matrix-settings-card">
            <div class="agy-setting-row">
              <label class="agy-setting-label" for="agy-matrix-toggle">Включить эффект матрицы:</label>
              <input type="checkbox" id="agy-matrix-toggle" class="agy-checkbox">
            </div>
            <div class="agy-setting-row">
              <label class="agy-setting-label" for="agy-matrix-mode">Режим активности:</label>
              <select id="agy-matrix-mode" class="agy-select">
                <option value="dynamic">Динамический (IDLE + TASK_RUNNING)</option>
                <option value="task_only">Только во время задач (TASK_RUNNING)</option>
              </select>
            </div>
            <div class="agy-setting-row">
              <label class="agy-setting-label" for="agy-matrix-intensity">Интенсивность дождя:</label>
              <select id="agy-matrix-intensity" class="agy-select">
                <option value="low">Низкая (Subtle)</option>
                <option value="medium">Средняя (Balanced)</option>
                <option value="high">Высокая (Cyberpunk)</option>
              </select>
            </div>
            <div class="agy-setting-row" style="margin-top: 4px;">
              <button type="button" class="agy-btn-burst" id="agy-matrix-test-burst">⚡ Протестировать всплеск матрицы</button>
            </div>
          </div>

          <div class="agy-modal-section-title">
            <span>⚡</span> Скилы и команды контекстного меню
          </div>
        ...
        <div class="agy-modal-footer">
          <button class="agy-btn-add" id="agy-btn-reset-skills">Сбросить скилы</button>
    ...
    const burstBtn = document.getElementById('agy-matrix-test-burst');
    if (burstBtn) {
      burstBtn.addEventListener('click', () => {
        if (window.AgyMatrixEffect && typeof window.AgyMatrixEffect.triggerBurst === 'function') {
          window.AgyMatrixEffect.enable();
          window.AgyMatrixEffect.triggerBurst(25);
          const toggle = document.getElementById('agy-matrix-toggle');
    ...
    document.getElementById('agy-btn-save-skills').addEventListener('click', () => {
      ...
      // 2. Сохранение настроек матрицы
      const toggle = document.getElementById('agy-matrix-toggle');
      const mode = document.getElementById('agy-matrix-mode');
      const intensity = document.getElementById('agy-matrix-intensity');

      if (toggle && mode && intensity) {
        const matrixCfg = {
        ...
        try {
          const current = localStorage.getItem('agy_matrix_effect_config');
          const merged = Object.assign({}, current ? JSON.parse(current) : {}, matrixCfg);
          localStorage.setItem('agy_matrix_effect_config', JSON.stringify(merged));
        } catch (e) {}
        ...
        if (window.AgyMatrixEffect) {
          ...
          if (typeof window.AgyMatrixEffect.setMode === 'function') {
            window.AgyMatrixEffect.setMode(matrixCfg.mode);
          }
          if (typeof window.AgyMatrixEffect.setIntensity === 'function') {
            window.AgyMatrixEffect.setIntensity(matrixCfg.intensity);
          }
        }
        const label = document.getElementById('agy-matrix-status-label');
        if (label) label.textContent = matrixCfg.enabled ? 'Вкл' : 'Выкл';
      }
  ...
  function bindMenuEvents(menu) {
    ...
    // Переключение эффекта матрицы
    const toggleMatrixItem = document.getElementById('agy-action-toggle-matrix');
    if (toggleMatrixItem) {
      toggleMatrixItem.addEventListener('click', () => {
        hideMenu();
        if (window.AgyMatrixEffect && typeof window.AgyMatrixEffect.toggle === 'function') {
          const isEnabled = window.AgyMatrixEffect.toggle();
          const label = document.getElementById('agy-matrix-status-label');
          if (label) label.textContent = isEnabled ? 'Вкл' : 'Выкл';
        }
      });
    }
  ...
  function init() {
    ...
    window.addEventListener('contextmenu', (e) => {
      ...
      // Обновление статуса матрицы в меню
      const matrixLabel = document.getElementById('agy-matrix-status-label');
      if (matrixLabel) {
        let isEnabled = true;
        if (window.AgyMatrixEffect && typeof window.AgyMatrixEffect.getState === 'function') {
          isEnabled = !!window.AgyMatrixEffect.getState().enabled;
        } else {
          try {
            const saved = localStorage.getItem('agy_matrix_effect_config');
            if (saved) isEnabled = !!JSON.parse(saved).enabled;
          } catch (e) {}
        }
        matrixLabel.textContent = isEnabled ? 'Вкл' : 'Выкл';
      }
